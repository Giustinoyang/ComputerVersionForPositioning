...
// Further modified to use libcvmatio to get exact edge and gradient data from MAT file
...
int main(int argc, char **argv) {
    if(argc == 1) {
        cout << "Usage: ./edge_boxes mat_file" << endl;
        return -1;
    }
   ...
   // if(!ok) {
   //     cout << "Error reading MAT file" << endl;
   //     return -1;
   // }
    ...
    Mat im, ime, ime_t, grad_ori, grad_ori_t;
    for(int i = 0; i < variables.size(); i++) {
    	if(variables[i].name().compare("I") == 0) im = variables[i].data<Mat>();
