...
vector<distcos> find_kNN(int k, vector<double>ln, vector<typeOfGraph> gr){
    ...
    while (j<vc.size()){
        ...
        for (i=0;i<gr[p].son.size();i++){
            ...
            //if (k==1) if (gr[p].son[i]==31) cout<<"this is 31:"<<ddd<<endl;
            if(ddd<vc[j].dist) {
                dd.who = gr[p].son[i];
                dd.dist = ddd;
                b[dd.who]=false;
                //cout<<dd.who<<':'<<ddd<<' '<<endl;
                vc.push_back(dd);
            }
        }
        j++;
            ...
            if (find_same(vf, vc)){
                /*if (k==1) {
                    ...
                    for (int kk=0;kk<vc.size();kk++)
                        cout<<vc[kk].who<<":"<<vc[kk].dist<<endl;
                }*/
                vc.resize(k);
                //cout<<endl;
                return vc;
